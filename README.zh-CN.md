...
用 React 式的组件、Hooks、Props、路由、输入层、主题与全局状态，在终端里构建声明式、响应式、异步的 TUI —— 构建于 Ratatui 与 Tokio 之上。
...
[![Website](https://img.shields.io/badge/website-ratatui--kit-3c8cba)](https://yexiyue.github.io/ratatui-kit/zh-cn/)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue)](LICENSE)

**[文档](https://yexiyue.github.io/ratatui-kit/zh-cn/start/)** ·
**[快速开始](https://yexiyue.github.io/ratatui-kit/zh-cn/start/quick-start/)** ·
**[组件](https://yexiyue.github.io/ratatui-kit/zh-cn/components/)** ·
**[示例](https://yexiyue.github.io/ratatui-kit/zh-cn/examples/)** ·
**[生态](https://github.com/yexiyue/awesome-ratatui-kit)** ·
**[English](README.md)**
...
- **协调与状态保留** —— 区分 Element（轻量声明，每帧重建）、Component（行为）、InstantiatedComponent（持久树节点）；按 `ElementKey` + `TypeId` 复用上一帧节点，**同 key 同类型即保留 Hooks 与状态**（类似 React 的 key diff）。
...
- **统一主题** —— 共享 `Palette` 是唯一颜色真源，每个组件经各自的 `FooTheme` 据它派生样式。用 `PaletteProvider` 全局换色、用 `ThemeOverride` 覆盖单一组件类型，或交给 `Atom` 驱动实现运行时换肤；协议 always-on、零额外依赖。
...
- **两套状态体系** —— 局部 `use_state`（每组件独立，随卸载释放）与全局 `Atom` / `use_atom`（进程级，`atom` 特性）；`State` / `AtomState` 均 `Copy` 且重载算术运算符，`count += 1` 即触发变更通知。
...
- **特性门控、按需裁剪** —— 默认零特性（`default = []`），`router` / `atom` / `input` / `tree` / `table` / `virtual-list` / `serde` 按需开启，只为你用到的能力付出体积。
...
> [!NOTE]
> 主库默认不启用任何特性。`router` / `atom` / `input` / `tree` / `table` / `virtual-list` 可按需开启，`full` 一次性启用全部。`textarea` 特性已随 Ratatui 0.30 迁移暂时下线（`tui-textarea` 暂无 0.30 兼容版）。
...
> README 不罗列完整 API，详细签名与用法见[官方文档站](https://yexiyue.github.io/ratatui-kit/zh-cn/)与 [docs.rs](https://docs.rs/ratatui-kit)。
...
| `PaletteProvider` / `ThemeOverride` | 主题注入 —— 全局调色板与组件级覆盖 | 核心 |
| `Input` / `SearchInput` | 文本输入 / 搜索框 | `input` |
| `TreeSelect` | 树形选择 | `tree` |
| `Table` | 数据驱动表格，支持单元格网格边框、换行、响应式列、页脚和行/列高亮 | `table` |
...
| `use_palette` / `use_component_theme` | 读取当前 Palette / 解析后的组件主题 | 核心 |
...
| `table` | `Table`、视宽感知换行、响应式列和单元格网格边框 | `unicode-width` |
| `virtual-list` | `VirtualList` 与 `tui_widget_list` re-export | `tui-widget-list` |
| `serde` | `Palette` 的 `Serialize` / `Deserialize` | `serde`、`ratatui/serde` |
| `full` | 以上全部 | — |
...
- [学习路径首页](https://yexiyue.github.io/ratatui-kit/zh-cn/start/)
- [快速开始](https://yexiyue.github.io/ratatui-kit/zh-cn/start/quick-start/)
- [安装与功能门控](https://yexiyue.github.io/ratatui-kit/zh-cn/start/installation/)
- [Hooks](https://yexiyue.github.io/ratatui-kit/zh-cn/core/hooks/)
- [状态模型](https://yexiyue.github.io/ratatui-kit/zh-cn/core/state/)
- [主题](https://yexiyue.github.io/ratatui-kit/zh-cn/core/theming/)
- [路由](https://yexiyue.github.io/ratatui-kit/zh-cn/core/routing/)
- [内置组件](https://yexiyue.github.io/ratatui-kit/zh-cn/components/)
- [示例](https://yexiyue.github.io/ratatui-kit/zh-cn/examples/)
...
cargo run --example table          # 数据表格：换行、边框与高亮
...
部分示例需要对应特性（如 `input` / `tree` / `table` / `virtual-list` / `router`）；从仓库根运行示例已默认启用 `full`，无需额外 flag。
...
除内置组件外，ratatui-kit 也支持独立发布的第三方组件 crate。生态 crate 通常命名为 `ratatui-kit-<name>`，只依赖稳定的 [Extension API](./EXTENSION_API.md)，不需要合并进主仓库，因此核心可以保持小而清晰。

- **发现组件** —— 在 crates.io 搜索 [`ratatui-kit` keyword](https://crates.io/keywords/ratatui-kit)，或浏览 [awesome-ratatui-kit](https://github.com/yexiyue/awesome-ratatui-kit)。
- **编写组件** —— 使用 [component template](https://github.com/yexiyue/ratatui-kit-component-template) 脚手架：
  `cargo generate yexiyue/ratatui-kit-component-template`，并参考 [Component Guide](./COMPONENT_GUIDE.md)。
- **官方扩展** —— 放在 [ratatui-kit-contrib](https://github.com/yexiyue/ratatui-kit-contrib)，例如 `ratatui-kit-markdown`（Markdown / 代码块 / diff / blockquote / divider）。
