...
**相关文件**：`packages/ratatui-kit-macros/src/element.rs`（`parse_children` / `ControlFlow` / `to_extend`）、`packages/ratatui-kit-macros/src/adapter.rs`、`packages/ratatui-kit/src/components/text.rs`（`Text` 组件、`TextParagraph: From<&str>/Line/Text`）、`examples/core/control_flow.rs`
...
**相关文件**：`packages/ratatui-kit-macros/src/element.rs`、`examples/advanced/custom_provider.rs`
