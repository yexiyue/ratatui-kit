...
- **`references/`** — deeper references the assistant reads on demand: every built-in component with its exact props, every hook signature, the macro grammar, the events / state / routing model, and idioms for building polished UIs.
