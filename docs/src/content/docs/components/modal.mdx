...
`Modal` 本身不替你决定哪些键关闭弹窗。底层弹层适合复杂交互；普通确认、提示和快捷键帮助优先用内置的 [ConfirmModal](/ratatui-kit/components/confirm-modal/)、[AlertModal](/ratatui-kit/components/alert-modal/) 和 [ShortcutInfoModal](/ratatui-kit/components/shortcut-info-modal/)。
...
需要在弹窗里放长内容时，把 [ScrollView](/ratatui-kit/components/scroll-view/) 放进 `Modal` 子树即可；它的 `Current` handler 会自动归属弹窗 layer。
