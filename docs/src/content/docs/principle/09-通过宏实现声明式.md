...
- 解析类型、属性、子元素，支持递归嵌套。
- 支持一等控制流和 `{ expr }` 表达式插槽，便于条件渲染和批量生成。
...
在`ratatui-kit-macros/src/element.rs`中，实现宏的核心解析和展开逻辑，负责把宏输入的嵌套结构、属性、`{ expr }` 表达式等，转成最终的 Element 构造代码。
        ...
        if input.peek(Brace) {
            ...
            while !children_input.is_empty() {
                if children_input.peek(Brace) {
                    // 支持 { expr } 语法，插入表达式作为子节点
                    children.push(ParsedElementChild::Expr(children_input.parse()?));
                } else {
                    // 递归解析嵌套子元素
