...
这一页面向框架贡献者和需要调试运行时的人。日常写应用时，先读 [组件模型](/ratatui-kit/core/component-model/)、[Hooks](/ratatui-kit/core/hooks/) 和 [输入层](/ratatui-kit/core/input-layers/) 就够了；只有当你要改 `render/`、`component/`、`hooks/` 或 `input/` 时，才需要把这里的调用链读完整。
...
```text
packages/ratatui-kit/src/render/tree.rs
Tree::render_loop
Tree::render
Tree::update_once
Tree::draw_root
...
更多使用层面的解释见 [组件模型](/ratatui-kit/core/component-model/) 和 [控制流](/ratatui-kit/core/control-flow/)。
...
分发顺序见 [输入层](/ratatui-kit/core/input-layers/)。实现入口在：

```text
packages/ratatui-kit/src/input/mod.rs
InputRuntime::begin_frame
InputRuntime::dispatch
packages/ratatui-kit/src/hooks/use_input.rs
```
