...
`AnyElement` 是类型擦除版本，用来把不同组件类型放进同一个 children 列表。完整宏语法见 [声明式语法](/ratatui-kit/zh-cn/core/declarative-syntax/)。
...
完整示例见 [原生 Widget 桥接](/ratatui-kit/zh-cn/advanced/custom-widget/)。
...
静态小数组可以用下标；例如 [控制流语法](/ratatui-kit/zh-cn/core/control-flow/) 里的 `ROWS` 是固定数组，所以 `key: index` 是稳定的。
...
这也解释了一个重要边界：你可以在 `element!` 里放条件渲染，但不要把 hook 调用放进条件分支。Hook 顺序属于组件实例，必须每帧稳定；渲染分支属于声明层，可以每帧变化。更多规则见 [Hooks](/ratatui-kit/zh-cn/core/hooks/)。
...
- 想理解 `if / for / match` 怎么写进树里，看 [控制流语法](/ratatui-kit/zh-cn/core/control-flow/)。
- 想查 `element!`、`#[component]`、Props 和 adapter 规则，看 [声明式语法](/ratatui-kit/zh-cn/core/declarative-syntax/)。
- 想理解状态为什么能跨帧保存，看 [Hooks](/ratatui-kit/zh-cn/core/hooks/) 和 [状态](/ratatui-kit/zh-cn/core/state/)。
- 想接入原生 ratatui widget，看 [原生 Widget 桥接](/ratatui-kit/zh-cn/advanced/custom-widget/)。
- 想理解整个 update / draw 循环，看 [渲染循环](/ratatui-kit/zh-cn/internals/render-loop/)。
