...
trybuild 编译测试 SHALL 覆盖全部公开宏：`element!`、`#[component]`、`#[derive(Props)]`、`routes!`、`#[with_layout_style]`。每个宏 MUST 至少有一个「正常用法编译通过」的用例。
...
运行时的可纯逻辑测试部分 SHALL 有 `#[test]` 覆盖，至少包括：`ElementKey`（`Decl`/`User` 不互相碰撞、`Hash`/`Eq` 自洽）、`use_state`/`atom` 的运算符重载与 `Copy` 语义、`router` 路径匹配（段边界匹配、命名参数提取）、`history` 的越界行为。
